            ...
            if($_SERVER['REQUEST_METHOD'] == "GET"){
                ...
                if(empty($errores)){
                    ...
                    if($_GET['ventas'] <= 1){
                        $_GET['ventas'] = 1;
                        $limiteInicial = 0;
                    }else if($_GET['ventas'] > $total){
                        $_GET['ventas'] = 1;
                        $limiteInicial = 0;
                    }else{
                        $limiteInicial = abs(($_GET['ventas'] - 1) * 8);            
                    }
                    $datas = $this->model->ventas($fechaInicio, $fechaMaxima, $productos, $categoria,$limiteInicial,8);
                    ...
                    $datas = $this->model->ventas("", "", 0, 0,$limiteInicial,8);
                ...
                $datas = $this->model->ventas("", "", 0, 0,$limiteIncial,8);
                $data = Caratula::caratula("Gestor de ventas", false, true, true, "Admin", $datas);
                $dataDos = Caratula::caratulaFiltrar($data, $categorias, 0, 0, "", "",$totalVentas);
                $this->view("ventas", $dataDos);
            }
        ...
        if($sesion->getLogin()){
            if($_SERVER['REQUEST_METHOD'] == "GET"){
                ...
                if($_GET['grafica'] <= 1){
                    $_GET['grafica'] = 1;
                    $limiteInicial = 0;
                }else if($_GET['grafica'] > $total){
                    $_GET['grafica'] = 1;
                    $limiteInicial = 0;
                }else{
                    $limiteInicial = abs(($_GET['grafica'] - 1) * 8);            
                }
                if(empty($errores)){
                    $datas = $this->model->ventasGrafica($fechaInicio, $fechaMaxima, $productos, $categoria,$limiteInicial,8);
                    $data = Caratula::caratula("Gestor de ventas", false, true, true, "Admin", $datas);
                    ...
                    $datas = $this->model->ventasGrafica("", "", 0, 0,$limiteInicial,8);
                    $data = Caratula::caratula("Gestor de ventas", false, true, true, "Admin", $datas, $errores);
